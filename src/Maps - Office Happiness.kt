...
fun main() {
    val meetingRoom = mapOf("tim" to 2, "jim" to 4, "randy" to 0, "sandy" to 5, "andy" to 8, "katie" to 6, "laura" to 2, "saajid" to 2, "alex" to 3, "john" to 2, "mr" to 8)
    val bossName = "john"

    print(outed(meetingRoom, bossName))
}

fun outed(meet: Map<String, Int>, boss: String): String {
    val averageScore = (meet.values.sum() + meet[boss]!!) / meet.count()
    return if (averageScore <= 5 ) "Get Out Now!" else "Nice Work champ!"
}