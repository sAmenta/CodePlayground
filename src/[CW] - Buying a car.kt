...
per month. Furthermore, this percent of loss increases of 0.5 percent at the end of every two months.
...
considered month but if, by chance from the start, the value of the old car is bigger than the value of the new one
...
fun main() {
    val result = nbMonths(8000, 8000, 1000, 1.5)
    println(result)
}
...
fun nbMonths(
    ...
    var monthCounter = 0
    var amountAvailable = -1.0
    var savings = savingPerMonth
    var newCarDecreasedValue = startPriceNew.toDouble()
    var oldCarDecreasedValue = startPriceOld.toDouble()
    var increasedLossPerMonth = percentLossByMonth
    while (amountAvailable < 0 && startPriceOld < startPriceNew) {
        monthCounter++
        if (monthCounter % 2 == 0) {
            increasedLossPerMonth += 0.5
        }
        newCarDecreasedValue -= (newCarDecreasedValue * increasedLossPerMonth) / 100
        oldCarDecreasedValue -= (oldCarDecreasedValue * increasedLossPerMonth) / 100
        amountAvailable = (oldCarDecreasedValue + savings) - newCarDecreasedValue
        savings += savingPerMonth
    }
    return if(startPriceOld >= startPriceNew) Pair(monthCounter, startPriceOld - startPriceNew)
    else Pair(monthCounter, amountAvailable.roundToInt())
}